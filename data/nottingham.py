...
class NottinghamDataset(Dataset):
    def __init__(self, training=True, aug=False, overfit=False, test_dir=None, logger=None):
        ### the min interval of duration, assume is 12
        self.names = []
        self.training = training
        self.aug = aug
        if self.training:
            self.root_path = Path(config.train_root_path)
            if overfit:
                self.root_path = Path(config.train_overfit_path)
        else:
            self.root_path = Path(config.val_root_path)
        if test_dir is not None and type(test_dir)==str:
            self.root_path = Path(test_dir)
